        ...
        Pass 
        {
           ...
           // float _Offset;
            ...
            float4 frag (Interpolaters i) : SV_Target
            {
                ...
                float xOffset = cos(i.uv.y * TAU * 0.2) * 0.1;
                float t = cos((i.uv.x + xOffset * 1) * TAU * 1) * 0.5 + 0.5;
               
                float4 outColor = lerp(_ColorA, _ColorB, t);
                float4 midColor = lerp(_ColorB, _ColorC, t);

                //float g = InverseLerp(_ColorStart, _ColorEnd, i.uv.x);

                return t  * outColor * midColor;
